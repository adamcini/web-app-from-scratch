...
with socket.socket() as server_sock:
    # Tell the kernel to reuse sockets that are in `TIME WAIT` state.
    server_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)

    # This tells the socket what address to bind to
    server_sock.bind((HOST, PORT))

    # Process one connection at a time and refuse additional connections
        ...
        with client_sock:
            try:
                ...
                if request.method != "GET":
                    client.sock.sendall(METHOD_NOT_ALLOWED_RESPONSE)
                    continue
